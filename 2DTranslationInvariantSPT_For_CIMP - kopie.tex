...
\begin{conjecture}\label{conj2}
	...
	induces a sequence (of group morphisms) on the stable equivalence classes. By this we mean that the class of $f(\phi)$ only depends on the class of $\phi$ and similarly for $g$. Moreover, this induced sequence is exact and split.
\end{conjecture}
Translation invariant SPT states in one spatial dimension are known to carry an $H^2(G,\TT)\oplus H^1(G,\TT)$ valued index (see appendix H of \cite{jappens2023spt}). From this and the last two conjectures, we expect there is an
...
Finally we will use $H^n(G,\TT)$ to denote the n-th (Borel) group cohomology of $G$ with coefficients in the torus $\TT$ (seen as a (group) module under addition with trivial group action). For the definition of these group cohomology modules we refer to \cite{benson1991representations}.
\section{Results}
...
\end{lemma}
\begin{proof}
	This is shown in appendix F of \cite{jappens2023spt}.
\end{proof}
...
\end{lemma}
\begin{proof}
	Since $\omega\circ\tau=\omega$ we get by the uniqueness of the GNS triple (see corollary A.2 of \cite{jappens2023spt}) that there exists a unique $\tilde{v}\in\UU(\HH_0)$ satisfying
	\begin{align}
...
\begin{definition}
	...
	\end{align}
	If additionally $y\in \textrm{Cone}_\sigma^W(\alpha_0,\theta)$ we will say that $x\in \IAC_\sigma^W(\alpha_0,\theta,\Theta)$.
\end{definition}
\begin{proof}
...
\end{lemma}
\begin{proof}
Using lemma A.3 of \cite{jappens2023spt} the result follows.
\end{proof}
...
\begin{proof}
	...
	\end{equation}
	Using lemma A.3 of \cite{jappens2023spt} there exists a $Z_{g,R}\in\UU(\HH_R)$ satisfying that $\Ad{W_g}(\Xi^R)=\id_{\HH_L}\otimes Z_{g,R}$. To show the second result we only need to observe that for any $A\in\UU(\AA_R)$ we get that
	\begin{align}
...
\end{lemma}
\begin{proof}
	The proof is presented after lemma B.1 of \cite{jappens2023spt}.
\end{proof}
\begin{lemma}
...
\begin{proof}
	...
	\end{equation}
	proving the result. The invariance of the three cochain is proven in lemma G.1 of \cite{jappens2023spt}.
\end{proof}
...
\end{lemma}
\begin{proof}
	This is proven in lemma G.2 of \cite{jappens2023spt}.
\end{proof}
...
\end{proof}
By the result of \cite{ogata2019classification} this implies that $\phi$ has a well defined 1d SPT index (see appendix H of \cite{jappens2023spt}). For any $j\in\ZZ$ and $n\in\NN$ take
\begin{align}
...
\end{lemma}
\begin{proof}
	The proof is in lemma C.1 of \cite{jappens2023spt}.
\end{proof}
...
\end{lemma}
\begin{proof}
	This follows from the fact that $\gamma^H_{0;1}\circ\gamma^{\Hsplit}_{1;0}\in \textrm{GVAut}_1(\AA)$ (see lemma J.4 of \cite{jappens2023spt} part 2).
\end{proof}
...
\end{lemma}
\begin{proof}
	The proof is presented after lemma D.1 of \cite{jappens2023spt}.
\end{proof}
...
\end{lemma}
\begin{proof}
	This proof is done under lemma  D.2 of \cite{jappens2023spt}.
\end{proof}
...
\end{lemma}
\begin{proof}
	This lemma is proven under lemma D.3 of \cite{jappens2023spt}.
\end{proof}
...
\begin{proof}
	...
	\end{align}
	Because of lemma J.5 of \cite{jappens2023spt} and lemma J.4 of \cite{jappens2023spt} part 2 there exists a $\tilde{\Theta}\in\Aut{\AA_{W(C_{\theta_2})^c}}$ and some $\tilde V_1,A_1\in\UU(\AA)$ such that
	\begin{align}
	...
	\end{align}
	If we now define the automorphism $\tilde{\beta}_{g,2}=\gamma^{H}_{1;0}\tilde\beta_{g,1}\gamma^{H}_{0;1}$ then this indeed satisfies that $\omega\circ\gamma^{H}_{0;1}\circ\tilde{\beta}_{g,2}=\omega\circ\gamma^{H}_{0;1}$. Define $\tilde\eta_g^\sigma\in\Aut{\AA_{C_{\theta_2}\cap\sigma}}$ through lemma J.7 of \cite{jappens2023spt}. It satisfies that there exists a $\tilde V_{2,g}\in\UU(\AA),A_{2,g}^{\sigma}\in\UU(\AA_\sigma)$ such that
	\begin{align}
...
\begin{align}
	...
	&\qquad(u^R_1(g,h)\otimes u^R_2(g,h))v^\dagger (u^R_1(g,h)\otimes u^R_2(g,h))^\dagger v\\
	&=C_1(g,h)C_2(g,h)
\end{align}
concluding the proof.
...
\begin{proof}
	...
	\end{equation}
	To do this one only has to use that $\beta_g^U$ commutes with $\nu$. By lemma A.3 of \cite{jappens2023spt} the result now follows.
\end{proof}
...
\end{lemma}
\begin{proof}
	The proof is presented after B.2 of \cite{jappens2023spt}.
\end{proof}
...
\begin{proof}
	...
	\end{equation}
	which would clearly conclude the proof. This is proven in lemma G.3 of \cite{jappens2023spt}.
\end{proof}
\begin{definition}
...
\end{lemma}
\begin{proof}
	The proof is given after lemma G.4 of \cite{jappens2023spt}.
\end{proof}
We will now show that the index is independent on the choice of $\tilde{\beta}_g$ and its decomposition.
...
\end{lemma}
\begin{proof}
	The proof of this lemma is given in lemma C.2 of \cite{jappens2023spt}.
\end{proof}
...
\end{lemma}
\begin{proof}
	This follows from the fact that because of J.4 of \cite{jappens2023spt} part 2 (or a slight modification thereof because of the new definition of $\HsplitTilde$ compared to the definition of $\Hsplit$.) $\gamma^H_{0;1}\circ\gamma^{\HsplitTilde}_{1;0}\in \textrm{GVAut}_1(\AA)$.
\end{proof}
...
\end{lemma}
\begin{proof}
	This lemma is proven under lemma E.1 of \cite{jappens2023spt}.
\end{proof}
...
\end{lemma}
\begin{proof}
	This lemma is proven in J.13 of \cite{jappens2023spt}.
\end{proof}
...
\end{lemma}
\begin{proof}
	The proof is presented after lemma E.2 of \cite{jappens2023spt}.
\end{proof}
...
\end{lemma}
\begin{proof}
	This lemma is proven after lemma E.4 of \cite{jappens2023spt}. The proof of E.4 hinges on lemma E.3. We also remark that this last lemma is the only place where the compactness of the group is used in this paper.
\end{proof}
...
\begin{proof}
	...
	\end{align}
	Because of lemma J.5 and J.4 of \cite{jappens2023spt} part 2 there exists a $\tilde{\Theta}\in\Aut{\AA_{W(C_{\theta_2})^c}}$ and some $\tilde V_1,A_1\in\UU(\AA)$ such that
	\begin{align}
	\alpha\circ\gamma^H_{0;1}&=\Ad{\tilde V_1}\circ(\alpha_L\circ\gamma^{H_L}_{0;1}\otimes\alpha_R\circ\gamma^{H_R}_{0;1})\circ\tilde\Theta\\
	\label{eq:DefinitionOfA1TwoTranslations}
	(\alpha_L\circ\gamma^{H_L}_{0;1}\otimes\alpha_R\circ\gamma^{H_R}_{0;1})\circ\tilde\Theta&=\Ad{A_1}\circ \alpha_L\otimes\alpha_R\circ\Theta\circ\gamma^{H}_{0;1}.
	\end{align}
	If we now define the automorphism $\tilde{\beta}_{g,2}=\gamma^{H}_{1;0}\circ\tilde\beta_{g,1}\circ\gamma^{H}_{0;1}$ then this indeed satisfies that $\omega\circ\gamma^{H}_{0;1}\circ\tilde{\beta}_{g,2}=\omega\circ\gamma^{H}_{0;1}$. Define $\tilde\eta_g^\sigma$ through lemma J.12 of \cite{jappens2023spt}. It satisfies that there exists a $\tilde V_{2,g}\in\UU(\AA),A_{2,g}^{\sigma}\in\UU(\AA_{\nu^{\sigma}(\sigma)})$ such that
	\begin{align}
...
\end{lemma}
\begin{proof}
	Completely analogous to the proof of (for example) J.4 of \cite{jappens2023spt}.
\end{proof}
...
\begin{proof}
	...
	\end{align}
	By appendix A.3 of \cite{jappens2023spt}, there exist $\tilde{X}_g\in\UU(\HH_{0,1}\otimes\HH_{0,2})$ and $\tilde{Y}_g\in\UU(\HH_{0,3}\otimes\HH_{0,4})$ such that
	\begin{equation}
...
\begin{lemma}
	...
	\begin{align*}
		\tilde{b}_g^UK_g^R X_g&=v^\dagger X_g v\Label{eq:H^1IndexRotationInvariantFirstDefinition}&
...
\begin{proof}
	...
	\end{align}
	where the $\pi_{0,i}$ are irreducible representations in $\AA_i$, the areas $i$ are indicated in figure \ref{fig:IndexInvariantUnderRotationDecomposition} and the $\Theta$ have support in the areas shaded red. This shows that up to some inner automorphisms the operators with an $R$ on top act on different parts of the decomposition of $\pi_0$ then the operators with a $U$ on top do. Combining this with lemma A.3 of \cite{jappens2023spt} concludes the proof.
\end{proof}
\begin{lemma}
