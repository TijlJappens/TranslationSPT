...
\end{lemma}
\begin{proof}
	We will only prove the first item as the second one is completely analogous. $\forall n\in\NN,$ take $B_n\subset\ZZ$ to be the ball with radius $n$, take
	\begin{equation}
		u_n\defeq \mathcal{T}\exp(-i\int_0^1 \d s \sum_{I\subset B_n}H(s,I))
	\end{equation}
	and take similarly
	\begin{equation}
		v_n\defeq \mathcal{T}\exp(-i\int_0^1 \d s \sum_{I\subset B_n}H_{(C_{\theta_1}\cup \tau(C_{\theta_1}))^c}(s,I)).
	\end{equation}
	We claim that there exists an $a_1\in\UU(\AA)$ such that
	\begin{equation}
		\lim_{n\rightarrow\infty}\norm{u_n^\dagger\Theta(u_n v_n^\dagger)v_n-a_1}=0.
	\end{equation}
	...
	\begin{equation}
		w_n\defeq \mathcal{T}\exp(-i\int_0^1 \d s \sum_{I\subset B_n}H_{C_{\theta_1}\cup \tau(C_{\theta_1})}(s,I)).
	\end{equation}
	This has the property that $\Theta(w_n)=w_n$. Take $n_0$ then for any $n>m>n_0$ we have that
	\begin{align}
		&\norm{u_n^\dagger\Theta(u_nv_n^\dagger)v_n-u_m^\dagger\Theta(u_mv_m^\dagger)v_m}\\
		&=\norm{u_n^\dagger\Theta(u_nv_n^\dagger w_n^\dagger)w_nv_nu_n^\dagger u_n-u_m^\dagger\Theta(u_mv_m^\dagger w_m^\dagger)w_mv_mu_m^\dagger u_m}\\
		&=\norm{\Theta(u_nv_n^\dagger w_n^\dagger)w_nv_nu_n^\dagger -u_n u_m^\dagger\Theta(u_mv_m^\dagger w_m^\dagger)w_mv_mu_m^\dagger u_mu_n^\dagger}\\
		&\leq \norm{\Theta(u_nv_n^\dagger w_n^\dagger)w_nv_nu_n^\dagger-\Theta(u_mv_m^\dagger w_m^\dagger)w_mv_mu_m^\dagger}\\
		&\quad +\norm{[\Theta(u_mv_m^\dagger w_m^\dagger)w_mv_mu_m^\dagger,u_mu_n^\dagger]}.
	\end{align}
