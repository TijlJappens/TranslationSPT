...
\begin{proof}
	...
	\begin{align}
		...
		&=v^\dagger w^\dagger\delta^R_g w b_{g,1}^R (\delta^R_g)^\dagger v h_g v^\dagger \delta_g^R v K_{g,1}^R (\delta_g^R)^\dagger \delta_g^R (b_{1,g}^R)^\dagger w^\dagger (\delta_g^R)^\dagger w w^\dagger \delta_g^R (K_{g,1}^R)^\dagger v^\dagger (\delta_g^R)^\dagger v  w
	\end{align}
\end{proof}
We will now show that the index is independent on the choice of $\tilde{\beta}_g$ and its decomposition.
