...
\end{proof}
Notice that $u_\sigma(g,h)$ and $K^\sigma_g$ are representations in the GNS space of automorphisms that have support in the widened cones. We now define a subgroup of $\UU(\pi_0\circ\alpha_0\circ\Theta(\AA)'')$ that includes the representations of cone automorphisms:
\begin{definition}
	Take $0<\theta<\pi/2$ and $\alpha_\sigma\in\Aut{\AA_{\sigma}}$. Take $u\in\UU(\pi_0\circ\alpha_0\circ\Theta(\AA)'')$ then we say that $u$ is a cone operator on the right/left (or in short $u\in\textrm{Cone}_{\sigma}(\alpha_0,\theta)$ for $\sigma\in\{L,R\}$) if there exists a $\xi\in\Aut{(\AA_{C_\theta}\cup\tau(\AA_{C_\theta}))\cap\AA_\sigma}$ such that
	\begin{equation}
		\Ad{u}\circ\pi_0\circ\alpha_0=\pi_0\circ\alpha_0\circ\xi.
	\end{equation}
	the $\textrm{Cone}_\sigma(\alpha_0,\theta)$ are both subgroups of $\UU(\pi_0\circ\alpha_0\circ\Theta(\AA)'')$.
\end{definition}
Notice that both $u_\sigma(g,h)$ and $K_g^\sigma$ are elements of $\textrm{Cone}_\sigma(\alpha_0,\theta)$. We now define a subgroup of $\UU(\pi_0\circ\alpha_0\circ\Theta(\AA)'')$ that includes both the group $\UU(\pi_0\circ\alpha_0\circ\Theta(\AA_R))$ and the representations of cone automorphisms:
\begin{definition}
	...
	\end{equation}
	$\textrm{IAC}_R$ is a subgroup of $\UU(\pi_0\circ\alpha_0\circ\Theta(\AA)'')$. If we define similarly $\textrm{IAC}_L$ we get that
...
\begin{lemma}\label{lem:AdjointOverConeIsInCone}
	Take $\Gamma^{\sigma}\in\textrm{Cone}_\sigma(\alpha_0,\theta)$ (for all $\sigma\in\{L,R\}$) then $\Ad{W_g}(\Gamma^\sigma)\in\textrm{Cone}_\sigma(\alpha_0,\theta)$.	If more generally we take $\Gamma^\sigma\in\textrm{IAC}_\sigma(\alpha_0,\Theta)$ then $\Ad{W_g}(\Gamma^\sigma)\in\textrm{IAC}_\sigma(\alpha_0,\Theta)$ $\forall g\in G$.
\end{lemma}
\begin{proof}
	content...
\end{proof}
...
\begin{proof}
	...
	\end{equation}
	Since the last equation is split we can take $\delta_g^L\in\textrm{Cone}_L(\alpha_0,\theta)$ and $\delta_g^R\in\textrm{Cone}_R(\alpha_0,\theta)$ such that
	\begin{equation}
