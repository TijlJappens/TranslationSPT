...
\begin{definition}
	...
	\end{equation}
\end{definition}
We will often write $\alpha_0$ to mean $\alpha_L\otimes\alpha_R$. We will use this class of automorphisms to define the states for which we can define the index. Take $\omega\in\PP(\AA)$ to be such that
\begin{enumerate}
...
\end{proof}
Notice that $u_\sigma(g,h)$ and $K^\sigma_g$ are representations in the GNS space of automorphisms that have support in the widened cones. We now define a subgroup of $\UU(\HH_0)$ that includes the representations of cone automorphisms:
\begin{definition}
	Take $0<\theta<\pi/2$ and $\alpha_\sigma\in\Aut{\AA_{\sigma}}$. Take $u\in\UU(\HH_{R/L})\otimes\mathbbm{1}_{\HH_{L/R}}$ then we say that $u$ is a cone operator on the right/left (or in short $u\in\textrm{Cone}_{\sigma}(\alpha_0,\theta)$ for $\sigma\in\{L,R\}$) if there exists a $\xi\in\Aut{(\AA_{C_\theta}\cup\tau(\AA_{C_\theta}))\cap\AA_\sigma}$ such that
	\begin{equation}
		\Ad{u}\circ\pi_0\circ\alpha_0=\pi_0\circ\alpha_0\circ\xi.
	\end{equation}
...
\begin{proof}
	...
	\end{align}
\end{proof}
Notice that both $u_\sigma(g,h)$ and $K_g^\sigma$ are elements of $\textrm{Cone}_\sigma(\alpha_0,\theta)$. We now define a subgroup of $\UU(\HH_0)$ that includes both the group $\UU(\pi_0\circ\alpha_0\circ\Theta(\AA_R))$ and the representations of cone automorphisms:
\begin{definition}
	Take $0<\theta<\pi/2$, $\alpha_\sigma\in\Aut{\AA_{\sigma}}$ and $\Theta\in\Aut{(\AA_{C_\theta}\cup\tau(\AA_{C_\theta}))^c}$. Take $u\in\UU(\pi_0\circ\alpha_0\circ\Theta(\AA)'')$ then we say that $u$ is an inner after cone operator on the right (or in short $u\in \textrm{IAC}_R(\alpha_0,\theta,\Theta)$) if there exists an $A\in\AA_R$ and a $v\in\textrm{Cone}_R(\alpha_0,\theta)$ such that
	\begin{equation}
		u=\pi_0\circ\alpha_0\circ\Theta(A)v.
	\end{equation}
