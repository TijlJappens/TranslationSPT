...
\begin{proof}
	...
	\begin{enumerate}
		...
		\begin{equation}
			\phi(\pi_0\circ\alpha_0\circ\Theta(A))=\pi_0\circ\alpha_0\circ\Theta\circ\gamma\circ\gamma_{H_\sigma}^{-1}(A).
		\end{equation}
		\item $\pi(v)=v.$
	...
	\end{align}
	where $\delta^\sigma_g=\pi_0\circ\alpha_0\circ\Theta\circ\gamma_{H_\sigma}(A^\sigma_{2,g}).$ The fact that the index is invariant under $\Ad{\pi_0(A_1)}$ and under $\phi(\cdot)$ should be rather clear. The proof that it is invariant under the transformation with de $\delta^\sigma_g$ is less straightforward. This proof should be analogous to the proof of lemma \ref{lem:InvarianceUnderChoiceBeta} with one clear difference. The tensor product in the proof is not taken over $\BB(\HH_L)\otimes\BB(\HH_R)$ like in the previous proof\footnote{there the $\delta^\sigma_g$ was in $\pi_0\circ\alpha_0\circ\Theta(\AA_{C_\theta})''=\pi_0\circ\alpha_0(\AA_{C_\theta})''\subset \HH_\sigma$} but in stead it is over $\pi_0\circ\alpha_0\circ\Theta(\AA_L)''\otimes \pi_0\circ\alpha_0\circ\Theta(\AA_R)''{\color{red}=?\BB(\HH)}$. The actual calculations however should be the same.
\end{proof}
\bibliography{TSPT}
