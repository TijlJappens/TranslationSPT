...
\begin{definition}
	Take $0<\theta<\pi/2$ and $\alpha_L\in\Aut{\AA_{L}}$ and $\alpha_R\in\Aut{\AA_{R}}$. Take $\sigma\in\{L,R\}$ and $x\in\UU(\HH_{\sigma})\otimes\mathbbm{1}_{\HH_{\ZZ^2/\sigma}}$ then we say that $x$ is a cone operator on $\sigma$ (or in short $x\in\textrm{Cone}_{\sigma}(\alpha_0,\theta)$) if and only if there exists a $\xi\in\Aut{W(\AA_{C_\theta})\cap\AA_\sigma}$ such that
	\begin{equation}
		\Ad{x}\circ\pi_0\circ\alpha_0=\pi_0\circ\alpha_0\circ\xi.
	\end{equation}
	$\textrm{Cone}_\sigma(\alpha_0,\theta)$ is a subgroup of $\UU(\HH_\sigma)\otimes\mathbbm{1}_{\ZZ^2/\sigma}$. We have that
...
\begin{equation}
	\AA=\overline{\bigoplus_{j\in\ZZ}(\BB_{\text{loc}})_j}.
\end{equation}
...
\end{align}
Define $\omega\in\PP(\AA)$ to be such that for any $A\in\AA$:
\begin{equation}
	\omega(A)=\sum_{j\in\ZZ}\phi(\Pi_{L_j}(A)).
\end{equation}
(Here $\Pi$ is the conditional expectation value.) We have that:
\begin{lemma}
	$\omega\in\PP(\AA)$ and $\omega$ satisfies assumption \ref{assumption}.
\end{lemma}
\begin{proof}
	...
	\begin{equation}
		\omega_0\defeq \omega\circ\alpha=\sum_{j\in\ZZ}\phi_0(\Pi_{L_j}(\cdot))
	\end{equation}
	is clearly a product state. We will now show that $\alpha\in\textrm{QAut}(\AA)$. To do this, take $0<\theta<\pi/2$ and define $\Theta\in\Aut{\AA_{W(C_\theta)^c}}$ to be
	\begin{equation}
...
\end{proof}
This shows that $\omega$ has a well defined 2d translation index. We will now show that $\textrm{Index}(\omega)=\textrm{Index}_1(\phi)$. We now fix a GNS triple of $\omega_0$ that is of the form
\begin{equation}
	\left(\HH_0=\bigotimes_{\substack{\sigma\in\{L,R\}\\ \rho\in\{L_j|j\in\ZZ\}}}\HH_{\rho,\sigma},\pi_0=\bigotimes_{\substack{\sigma\in\{L,R\}\\ \rho\in\{L_j|j\in\ZZ\}}}\pi_{\rho,\sigma},\Omega_0=\bigotimes_{\substack{\sigma\in\{L,R\}\\ \rho\in\{L_j|j\in\ZZ\}}}\Omega_{\rho,\sigma}\right)
\end{equation}
where the $\pi_{\rho,\sigma}:\AA_{\rho\cap\sigma}\rightarrow\BB(\HH_{\rho,\sigma})$ are all irreducible representations.
\begin{remark}
	We clearly have (by construction) that (for any $j\in\ZZ$)
	\begin{equation}
		\pi_{L_j,L}\circ\alpha_L|_{L_j}\otimes \pi_{L_j,R}\circ\alpha_R|_{L_j}
	\end{equation}
	is a gns triple of $\phi$. This implies that if we take any $k_{j}^R(g)$ such that
	\begin{equation}
		\Ad{k_{j}^R(g)}\circ\pi_{L_j,R}\circ\alpha_R|_{L_j}=\pi_{L_j,R}\circ\alpha_R|_{L_j}\circ\beta_{g}^{L_j\cap R}
	\end{equation}
	then there exists a 2 cochain $C$ satisfying that
	\begin{equation}
		k_{j}^R(g)k_{j}^R(h)k_{j}^R(gh)^{-1}=C(g,h)\mathbbm{1}_{\HH_0}.
	\end{equation}
\end{remark}
Take $\tilde{W}_g$ to be the unique operator such that
\begin{align}
	\Ad{\tilde W_g}\circ\pi_0&=\pi_0\circ\alpha\circ\beta_g^U\circ \alpha^{-1}&\tilde{W}_g\Omega_0&=\Omega_0.
\end{align}
Clearly this $\tilde{W}_g$ is a (linear) representation of the group. Now define $W_g=V_1^{\dagger}\tilde{W}_g V_1$. This operator is also a linear representation and satisfies the conditions indicated in lemma \ref{lem:Definition_W_And_u}. Since $W_g$ is a linear representation we get that $W_gW_hW_{gh}^{-1}=\mathbbm{1}_{\HH_0}$. This implies that we can take $u_\sigma(g,h)=\mathbbm{1}_{\bigotimes_{\rho\in\{L_j|j\in\ZZ\}}\HH_{\sigma,\rho}}$ (for any $\sigma\in\{L,R\}$). All that is now left is to define $K^\sigma_g\in\UU(\HH_\sigma)$. We do this as follows, take $k^\sigma_g\in\UU(\HH_{L_0,\sigma})$ to be such that
\begin{equation}
	\Ad{k^\sigma_g}\circ\pi_{L_0,\sigma}=\circ\pi_{L_0,\sigma}\circ\beta_g^{\sigma\cap L_0}.
\end{equation}
This operator is unique up to a $G$-dependent phase. Take $K^\sigma_g\defeq k^\sigma_g\bigotimes_{j\in\ZZ/\{0\}}\mathbbm{1}_{\HH_{L_j,\sigma}}$ then this operator indeed satisfies what was written in lemma \ref{lem:Definition_K}. We now only need to prove one more thing in order to be able to calculate the index. We need
\begin{equation}
	[K^\sigma_g\otimes\mathbbm{1}_{\HH_{\ZZ^2/\sigma}},v^\dagger W_g v]=0.
\end{equation}
Clearly the adjoint action of this commutator on the gns representation vanishes (this is because the two automorphisms commute) we therefore only have to show that this gives zero when applied to the cyclic vector. To show this, notice that by construction there exists a $w_g\in\UU(\otimes_{j>0}\HH_{L_j,L}\otimes\HH_{L_j,L})$ such that $v^\dagger W_g v=\mathbbm{1}_{\otimes_{j\leq 0}\HH_{L_j}}\otimes w_g$. We now get
\begin{equation}
	content...
\end{equation}
...
\begin{proof}
	...
	\begin{equation}
		\sum_{Z\subset\ZZ^2}\left(\Xi^{(s)}(Z,t)-\sum_{C\in\mathcal{C}_1}\mathbbm{1}_{Z\subset C}\Xi^{(s)}(Z,t)\right)
	\end{equation}
