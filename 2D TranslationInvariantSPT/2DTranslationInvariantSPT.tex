...
\begin{lemma}\label{lem:DefinitionOfGroupMorphism}
	Take $B_n\subset\ZZ^2$ the ball of radius $n$. Define
	\begin{equation}
		u_{\text{split},n}\defeq \pi_0\circ\alpha_0\circ\Theta\left(\mathcal{T}\exp(-i\int_0^1 \sum_{I\subset B_n} H_{L}(I))\otimes\mathcal{T}\exp(-i\int_0^1 \sum_{I\subset B_n} H_{R}(I))\right)
	\end{equation}
	and similarly
	\begin{equation}
		u_{n}\defeq \pi_0\circ\alpha_0\circ\Theta \left(\mathcal{T}\exp(-i\int_0^1 \sum_{I\subset B_n} H(I))\right).
	\end{equation}
	Take
	\begin{equation}
		\phi_1:\textrm{IAC}_R(\alpha_0,\Theta) \rightarrow \textrm{IAC}_R(\alpha_0,\Theta\circ \gamma \circ \gamma_L^{-1}\otimes\gamma_R^{-1}):x\mapsto\lim_{n\rightarrow\infty}u_nu_{\text{split},n}^{\dagger}xu_{\text{split},n}u_n^{\dagger}
	\end{equation}
	where the limit is defined in norm topology then this $\phi_1$ is well defined (the limit exists) and is a group homomorphism. Furthermore for our $\xi\in\Aut{\AA_{C_\theta}\cap\AA_R}$ and our $A\in\AA_R$ satisfying that
	\begin{equation}
...
\begin{proof}
	...
	\end{equation}
	the group homomorphism defined from lemma \ref{lem:DefinitionOfGroupMorphism}. Define (similarly)
	\begin{equation}
