...
In what follows let $\AA$ be a quasi-local $C^*$ algebra defined on a two dimensional square lattice. Let $G$ be a group and let $U_i\in\hom(G,\AA_i)$ be the on site group action. Take $\beta^\Gamma\in\hom(G,\Aut{\AA_\Gamma})$ (for any $\Gamma\subset\ZZ^2$) be such that
\begin{equation}
...
\begin{equation}
	F_\phi:r\mapsto \defeq \frac{\exp(-r^\phi)}{(1+r)^4}.
\end{equation}
...
\end{equation}
be a one parameter family of interactions such that the $F$-norm is uniformly bounded ($\sup_{t\in[0,1]}\norm{\Phi(\cdot,t)}_F<\infty$). We define the locally generated automorphism $\gamma^{\Phi}_{s;t}$ such that for any $A\in\AA$, $\gamma^{\Phi}_{s;t}(A)$ is given as the solution to the differential equation
\begin{equation}
	\frac{\d}{\d t}\gamma^{\Phi}_{s;t}(A)=-i\sum_{I\in\mathfrak{G}_{\ZZ^2}}\gamma^{\Phi}_{s;t}([\Phi(I,t),A])
\end{equation}
with initial condition $\gamma^{\Phi}_{s;s}(A)=A$.
\section{Statement of the result in therms of short range entanglement}
