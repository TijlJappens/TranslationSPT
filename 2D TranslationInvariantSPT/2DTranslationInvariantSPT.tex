...
\begin{proof}
	...
	\begin{equation}
		u_n(t_1;t_2)\defeq \mathcal{T}\exp(-i\int_{t_1}^{t_2} \d s \sum_{I\subset B_n}H(s,I))
	\end{equation}
	and take similarly
	\begin{equation}
		v_n(t_1;t_2)\defeq \mathcal{T}\exp(-i\int_{t_1}^{t_2} \d s \sum_{I\subset B_n}H_{(C_{\theta_1}\cup \tau(C_{\theta_1}))^c}(s,I)).
	\end{equation}
	When we don't explicitly write the times we mean $t_1=0$ and $t_2=1$. We claim that there exists an $a_1\in\UU(\AA)$ such that
	\begin{equation}
		\lim_{n\rightarrow\infty}\norm{\Theta(u_n v_n^\dagger)v_nu_n^\dagger-\gamma(a_1)}=0.
	\end{equation}
	...
	\begin{equation}
		w_n(t_1;t_2)\defeq \mathcal{T}\exp(-i\int_{t_1}^{t_2} \d s \sum_{I\subset B_n}H_{C_{\theta_1}\cup \tau(C_{\theta_1})}(s,I)).
	\end{equation}
	This has the property that $\Theta(w_n)=w_n$. Take $n_0$ then for any $n,m>n_0$ we have that
	\begin{align}
		\Delta&\defeq\Theta(u_nv_n^\dagger)v_nu_n^\dagger-\Theta(u_mv_m^\dagger)v_mu_m^\dagger\\
		&=\Theta(u_nv_n^\dagger w_n^\dagger)w_nv_nu_n^\dagger -\Theta(u_mv_m^\dagger w_m^\dagger)w_mv_mu_m^\dagger.
	\end{align}
	Define $\tilde{u}_n$ and $\tilde{H}_n$ as
	\begin{align}
		\tilde{u}_n(t_1;t_2)&\defeq \Theta(u_n(t_1;t_2)v_n(t_1;t_2)^\dagger w_n(t_1;t_2)^\dagger)w_n(t_1;t_2)v_n(t_1;t_2)u_n(t_1;t_2)^\dagger\\
