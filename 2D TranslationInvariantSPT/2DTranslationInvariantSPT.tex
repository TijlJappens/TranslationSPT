...
\end{enumerate}
One then defines an equivalence class on these spt states by saying that $\omega_1$ (a unique gapped groundstate of $\Phi_1$) is equivalent to $\omega_2$ (a unique gapped groundstate of $\Phi_2$) if there exists a $G$-invariant path of interactions (with a uniform lower bound on the gap) connecting $\Phi_1$ and $\Phi_2$. The space of equivalence classes of spt states is then called the spt classification. In one spacial dimension the simplest examples of non trivial spt states are so called matrix product states (\cite{pollman2012symmetry},\cite{schuch2011MatrixProduct}). Later on and more generally it was shown by Yoshiko Ogata \cite{ogata2019classification} that any state satisfying the split property (this includes both gapped groundstates of local Hamiltonians and invertible states) carries an $H^2(G,\TT)$-valued\footnote{By $H^n(G,\TT)$ we mean the n-th (Borel) group cohomology of $G$ with coefficients in the torus $\TT$ (seen as a ring under addition). Group cohomology is defined in section \ref{sec:GroupCohomology}.} index and that this index is invariant under this equivalence class. Later on Anton Kapustin, Bowen Yang and Nikita Sopenko \cite{kapustin2021classification} then showed that this classification problem is complete (the index is injective) if one looks at invertible states under stable equivalence. In two spatial dimensions more recently Yoshiko Ogata \cite{ogata2021h3gmathbb} proved there is an $H^3(G,\TT)$-valued index this time belonging to the classification problem as presented here that is protected under this equivalence class.
\\\\
We've already touched the concept of stacking in saying that it was required in the proof that the $H^2(G,\TT)$ index is complete. Stacking however plays also another role, it allows us to define an (abelian) group structure. This construction goes as follows. First we say that two spt states, $\omega_1$ and $\omega_2$ are stably equivalent if and only if there exist trivial\footnote{Trivial means that the ground state is a product state and that it transforms under the on site group action trivially.} spts, $\psi_1$ and $\psi_2$ such that $\omega_1\otimes_{\text{stack}}\phi_1$\footnote{The $\otimes_\text{stack}$ operation is defined in section \ref{sec:QuasiLocalC*Algebra}.} is equivalent to $\omega_2\otimes_{\text{stack}}\phi_2$. There is now a conjecture about the space of spt states under stable equivalence that says that the stacking operator induces a group structure:
\begin{conjecture}\label{conj:StableEquivGroupStructure}
	Let $*$ be the map
	\begin{equation}
		*:\{\text{stable spt classification}\}^2\rightarrow \{\text{stable spt classification}\}:(\expval{\omega_1}_\sim,\expval{\omega_2}_\sim)\mapsto \expval{\omega_1\otimes_\text{stack}\omega_2}_\sim.
	\end{equation}
	Then $*$ is well defined (independent of the choice of representative) and $\{\text{stable spt classification}\}$ together with the $*$ operation forms an abelian group.
\end{conjecture}
The goal of this paper is to look at the 2d case with the (stable) equivalence class as presented here but with one notable difference. We will only consider states and interactions that have a translation symmetry in one direction. There is a long standing conjecture about the spt classification of such states. See for instance \cite{xiong2019classification} section 3.1.5.1 where they present this conjecture in a very general way. It goes as follows:
\begin{conjecture}\label{conj}
	The space of translation invariant spts under stable equivalence\footnote{In the stable equivalence class for translation invariant states we ask that the path connecting two states is not only G-invariant but also translation invariant.} also satisfies conjecture \ref{conj:StableEquivGroupStructure}. Let $g$ be the inclusion map from the space of 2d translation invariant spt states to the more general set of 2d spt states. Let $f$ be the map that takes a 1d spt state and outputs a translation invariant 2d spt state by taking the tensor product in the direction of the translation symmetry. The sequence
	\begin{equation}
...
\begin{remark}
	...
	\end{align}
	This result can in turn be used to work out the Kuneth formula for $\ZZ^2\times G$
	\begin{equation}
...
\end{equation}
with initial condition $\gamma^{\Phi}_{s;s}(A)=A$ (the existence of this automorphism is proven in \cite{nachtergaele2010liebrobinson}). This satisfies the condition that if $\sup_t\norm{\sum_{I\subset\ZZ^2}\Phi(I,t)}<\infty$ that then
\begin{align}
...
\begin{theorem}
	...
	\begin{enumerate}
		...
		\end{equation}
		\item For $\phi$, any one dimensional short ranged entangled $G$-invariant state, the (infinite) tensor product of this state in the vertical direction (see section \ref{sec:OneTranslationDirectionExample} definition \ref{def:InfiniteTensorProductState}), $\omega$ satisfies
		\begin{equation}
...
\begin{theorem}
	...
	\begin{enumerate}
		...
		\begin{equation}
			\textrm{Index}_{1d\text{ trans}}^{\AA_{\text{1d}},U(g)}(\phi)=\text{Index}_{2\text{ trans}}^{\AA,U(g)}(\omega)
		\end{equation}
		where $\textrm{Index}_{1d\text{ trans}}^{\AA,U(g)}$ is defined in appendix \ref{sec:OneDimensionalIndices}.
...
\begin{theorem}\label{thrm:ExistenceFirstIndex}
	...
	\begin{enumerate}
		...
		\begin{equation}
			\textrm{Index}^{\AA,U(g)}(\omega)=\textrm{Index}^{\AA,U(g)}(\omega\circ\gamma^{\Phi}_{0;1}).
		\end{equation}
		\item For $\phi\in\PP(\AA_\text{1d})$, any a one dimensional $G$-invariant state satisfying assumption \ref{assumption1d}, the (infinite) tensor product of this state in the vertical direction (see section \ref{sec:OneTranslationDirectionExample} definition \ref{def:InfiniteTensorProductState}) satisfies
		\begin{equation}
			\textrm{Index}^{\AA_{\text{1d}},U(g)}_{1d}(\phi)=\textrm{Index}^{\AA,U(g)}(\omega).
		\end{equation}
...
\begin{theorem}\label{thrm:ExistenceSecondIndex}
	...
	\begin{enumerate}
		...
		\end{equation}
		\item For $\phi$, any a one dimensional $G$-invariant and translation invariant state satisfying assumption \ref{assumption1d}, the (infinite) tensor product of this state in the vertical direction (see section \ref{sec:OneTranslationDirectionExample} definition \ref{def:InfiniteTensorProductState}) which we now call $\omega$ satisfies
		\begin{equation}
...
\begin{proof}
	...
	\end{align}
\end{proof}
We will also define a generalisation of this. We will define a subgroup of $\UU(\HH_0)$ that includes both the group $\UU(\pi_0\circ\alpha_0\circ\Theta(\AA_R))$ and the representations of cone automorphisms:
\begin{definition}
...
\begin{proof}
	...
	\end{equation}
	Using lemma \ref{lem:SplittingOfUnitary} there exists a $Z_{g,R}\in\UU(\HH_R)$ satisfying that $\Ad{W_g}(\Xi^R)=\id_{\HH_L}\otimes Z_{g,R}$. To show the second result we only need to observe that for any $A\in\AA_R$ we get that
	\begin{align}
...
\begin{definition}
	Take $C:G^2\rightarrow U(1)$ to be the 2-cochain defined in \ref{lem:Definition2Cochain}. Take $\phi:G^2\rightarrow\TT$ such that $C(g,h)=\exp(i\phi(g,h))$ then we define the index as
	\begin{equation}
