...
\begin{proof}
	...
	and take similarly
	\begin{align}
		v_n(t_1;t_2)&\defeq \mathcal{T}\exp(-i\int_{t_1}^{t_2} \d s H_{n,\theta_1^c}(s) )&H_n(s)&=\sum_{I\subset B_n}H_{(C_{\theta_1}\cup \tau(C_{\theta_1}))^c}(s,I).
	\end{align}
	...
	\end{equation}
	Clearly by construction this $a_1$ satisfies equation \eqref{eq:TwoAngleLemmaEquation1}. We will show that this sequence is a Cauchy sequence. To do this, define the operator
	\begin{align}
	...
	\begin{align}
		\tilde{u}_n(t_1;t_2)&\defeq \Theta(u_n(t_1;t_2)^\dagger v_n(t_1;t_2) w_n(t_1;t_2))w_n(t_1;t_2)^\dagger v_n(t_1;t_2)^\dagger u_n(t_1;t_2)\\
	...
	\begin{align}
		&\tilde{H}_n(s)\tilde{u}_n(0;s)=\frac{\d}{\d s}\tilde{u}_n\\
		...
		&=\left(\Ad{\Theta(u_n(0;s)^\dagger v_n(0;s)w_n(0;s))w_n(0;s)^\dagger v_n(0;s)^\dagger}(\partial H_n(s))-\Ad{\Theta(u_n(0;s)^\dagger)\partial H_n(s)}\right)\tilde{u}_n(0;s).
	\end{align}
	First we will find a bound on
	\begin{align}
		&w_n(0;s)^\dagger v_n(0;s)^\dagger(\partial H_n(s)-\partial H_{n_0}(s))v_n(0;s)w_n(0;s)\\
		\nonumber
		&\quad-\Theta\left(w_n(0;s)^\dagger v_n(0;s)^\dagger(\partial H_n(s)-\partial H_{n_0}(s))v_n(0;s)w_n(0;s)\right).
	\end{align}
	{\color{red}Seems doable but I'm not fully sure how to do this. }What is left to bound is now
	\begin{align}
	&\Ad{\Theta(u_n(0;s)^\dagger v_n(0;s)w_n(0;s))w_n(0;s)^\dagger v_n(0;s)^\dagger}(\partial H_{n_0}(s))-\Ad{\Theta(u_n(0;s)^\dagger)\partial H_{n_0}(s)}\\
	\nonumber
	&\Ad{\Theta(u_m(0;s)^\dagger v_m(0;s)w_m(0;s))w_m(0;s)^\dagger v_m(0;s)^\dagger}(\partial H_{n_0}(s))-\Ad{\Theta(u_m(0;s)^\dagger)\partial H_{n_0}(s)}.
	\end{align}
	{\color{red}This only seems to be small when $m,n\gg n_0$. Maybe I should replace $n_0$ by something like $n_0/2$?}
\end{proof}
Another lemma that will be necessary:
\begin{lemma}\label{lem:GroupActionInCone}
	Take $\theta\in]0,\pi/2[$ then
	\begin{enumerate}
		\item there exists an $a\in\UU(\AA)$ such that
		\begin{equation}
