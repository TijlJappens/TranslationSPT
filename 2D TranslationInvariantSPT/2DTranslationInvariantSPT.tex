...
\begin{align}
	L&\defeq \left\{(x,y)\in\ZZ^2\left|x<0\right.\right\},&R&\defeq \left\{(x,y)\in\ZZ^2\left|x>0\right.\right\}\\
	U&\defeq \left\{(x,y)\in\ZZ^2\left|y>0\right.\right\},&D&\defeq \left\{(x,y)\in\ZZ^2\left|y<0\right.\right\}
\end{align}
...
\begin{enumerate}
	...
	\item there exist automorphisms $\alpha_L\in\Aut{\BB_L}$ and $\alpha_R\in\Aut{\BB_R}$ and operators $b\in\BB$ such that
	\begin{equation}
		\phi_0=\phi\circ\Ad{b}\circ\alpha_L\otimes\alpha_R
	\end{equation}
	is a product state.
\end{enumerate}
For any $j\in\ZZ$ take $L_j$ to be $\{(x,y)\in\ZZ^2|y=j\}$. Define $\omega\in\PP(\AA)$ to be such that for any $A\in\AA$:
\begin{equation}
	\omega(A)=\sum_{j\in\ZZ}\phi(\Pi_{L_j}(A)).
\end{equation}
(Here $\Pi$ is the conditional expectation value.) We have that $\omega\in\PP(\AA)$ and that $\omega$ satisfies assumption \ref{assumption}. Moreover $\textrm{Index}(\omega)=\textrm{Index}_1(\phi)$ {\color{red}to be continued.}
