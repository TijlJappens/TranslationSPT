...
The layout of this paper will be as follows: First we explain the setup, the concept of cone operators and using this we define our $H^2(G,\TT)$ valued index (although at this point the index is not only a function of the state but of many more choices). We then show that the index is well defined and does not depend on our choices. We then prove the fact that our index is in fact consistent with conjecture \ref{conj} and that indeed $\textrm{Index}(f(\omega))=\textrm{Index}_1(\omega)$. Finally we show that any equivalent spt states indeed have the same index. Finally in section \ref{sec:TwoDirectionTraslationInvariance} we construct the $H^1(G,\TT)$ valued index that appears when there are two translation symmetries.
\section{Setup and definitions}
In what follows let $\AA$ be a quasi-local $C^*$ algebra defined on a two dimensional square lattice (see eg. \cite{ogata2021h3gmathbb} section 1). Let $G$ be a group and let $U_i\in\hom(G,\AA_i)$ be the on site group action. Take $\beta^\Gamma\in\hom(G,\Aut{\AA_\Gamma})$ (for any $\Gamma\subset\ZZ^2$) be such that
\begin{equation}
...
\section{Results}
We will present our claim starting from two different assumptions. The first assumption is something related to short range entanglement whereas the second assumption is more technical but also more general (the first assumption will imply the second assumption).
\subsection{Statement of the results in therms of short range entanglement}
...
\begin{theorem}
	...
	\end{equation}
	that is well defined (doesn't depend on $\omega_0$ or the choice of disentangler). This map will be consistent with conjecture \ref{conj} by which we mean that
	\begin{enumerate}
		...
		\end{equation}
		\item Take $\phi$ a one dimensional short ranged entangled $G$-invariant state then the (infinite) tensor product of this state in the vertical direction (see section \ref{sec:OneTranslationDirectionExample}) satisfies
		\begin{equation}
...
\section{Example (consistency with conjecture \ref{conj})}\label{sec:OneTranslationDirectionExample}
...
\begin{proof}
	content...
\end{proof}
\section{The $H^1(G,\TT)$-valued index is invariant under choices}
