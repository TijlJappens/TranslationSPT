...
\begin{equation}
	C_\theta\defeq \{(x,y)\in\ZZ^2|\tan(\theta)\leq\frac{y}{x}\}
\end{equation}
...
\begin{proof}
	...
	\end{equation}
	Inserting this in equation \eqref{eq:IndexInvariantUnderLGA_A1_Transformation} now proves the fact that $\tilde{W}_g$ is an operator belonging to $\omega\circ\gamma^H_{0;1}$. {\color{red}I need to do the same for $\tilde u_R(g,h)$.}
\end{proof}
\appendix
...
\end{lemma}
\begin{proof}
	Part 3 follows from Theorem 5.2 in \cite{ogata2021h3gmathbb}, part 1 is done in Proposition 5.5 in \cite{ogata2021h3gmathbb} and part 4 follows trivially from Part 1. We therefore only have to comment on part 3. {\color{red}To be continued...}
\end{proof}
From these four statements we can prove the following results:
\begin{lemma}\label{lem:TwoAngleLemmaPart1}
	Take $\theta_1$ and $\theta_2$ such that $0<\theta_1<\theta_2<\pi/2$ then for all $\Theta\in\Aut{\AA_{(C_{\theta_2}\cup \tau(C_{\theta_2}))^c}}$ and $s,t\in\RR$ there exists an $a_1\in\UU(\AA)$ and a $\tilde{\Theta}\in \Aut{\AA_{(C_{\theta_1}\cup \tau(C_{\theta_1}))^c}}$ such that
