...
\begin{proof}
	...
	We now get that
	\begin{equation}
	\Theta(u_n^\dagger v_n)v_n^\dagger u_n-\Theta(u_m^\dagger v_m)v_m^\dagger u_m\leq \varepsilon_{n_0,1}+\varepsilon_{n_0,2}+\varepsilon_{n_0,3}+\varepsilon_{n_0,4}.
	\end{equation}
	Since for each $\varepsilon>0$ we can find an $n_0$ such that $\varepsilon_{n_0,1}+\varepsilon_{n_0,2}+\varepsilon_{n_0,3}+\varepsilon_{n_0,4}<\varepsilon$ the row is indeed a Cauchy row. As the metric is complete this implies that the row converges and hence the result follows.
\end{proof}
