...
\end{align}
where $I'$ is just $I$ shifted upwards. Take $\gamma_H:\RR\rightarrow\Aut{\AA}$ to be the locally generated automorphism generated from this interaction. We will denote by $\gamma_H\defeq \gamma_H(1)$. We will first need a lemma of the form
\begin{lemma}\label{lem:TwoAngleLemma}
	...
	\begin{enumerate}
		...
		\begin{equation}
		\gamma_{H_L}^{-1}\otimes\gamma_{H_R}^{-1}\circ\Theta_1\circ\gamma_H=\Ad{a_1}\circ\Theta_2.
		\end{equation}
		\item $\eta_{g,1}^{L/R}\in\Aut{\AA_{C_{\theta_1}}\cap\AA_{L/R}}$ there exists an $a_2\in\UU(\AA)$ and some $\eta_{g,2}^{L/R}\in\Aut{\AA_{C_{\theta_2}}\cap\AA_{L/R}}$ such that
		\begin{equation}
		\gamma_H^{-1}\circ\eta_{g,1}^L\otimes\eta_{g,1}^R\circ\gamma_H=\Ad{a_2}\circ \eta_{g,2}^L\otimes\eta_{g,2}^R.
		\end{equation}
	\end{enumerate}
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lem:GroupActionInCone}
	...
	\begin{equation}
	\gamma_H^{-1}\circ\beta_g^U\circ\gamma_H\circ(\beta_g^U)^{-1}=\Ad{a}\circ\eta^L_g\otimes\eta^R_g.
	\end{equation}
\end{lemma}
\begin{proof}
	This also looks like it has a standard Lieb-Robinson type argument.
\end{proof}
Finally before we can start the proof we also need:
\begin{lemma}
	Take $\theta\in]0,\pi/2[$ and 
\end{lemma}
\begin{theorem}
	$\textrm{index}(\omega)=\textrm{index}(\omega\circ\gamma_H)$
\end{theorem}
\begin{proof}
	...
	\end{equation}
	Take $W_g,u_{L/R}(g,h)$ and $K_g^{L/R}$ operators belonging to these automorphisms (see lemmas \ref{lem:Definition_W_And_u} and \ref{lem:Definition_K}). Clearly $\alpha\circ\gamma_H$ satisfies $\omega\circ\gamma_H=\omega_0\circ\alpha\circ\gamma_H$. We also have that $\alpha\circ\gamma_H\in\QAut{\AA}$. To show this notice that
	\begin{equation}
		\alpha\circ\gamma_H=\Ad{V_1}\circ(\alpha_L\circ\gamma_{H_L}\otimes\alpha_R\circ\gamma_{H_R})\circ\gamma_{H_L}^{-1}\otimes\gamma_{H_R}^{-1}\circ\Theta\circ\gamma_H.
	\end{equation}
	Because of lemma \ref{lem:TwoAngleLemma} there exists a $\tilde{\Theta}\in\Aut{\AA_{C_{\theta_2}}\cup\tau(\AA_{C_{\theta_2}})}$ and some $\tilde V_1,A_1\in\UU(\AA)$ such that
	\begin{align}
		\alpha\circ\gamma_H&=\Ad{\tilde V_1}\circ(\alpha_L\circ\gamma_{H_L}\otimes\alpha_R\circ\gamma_{H_R})\circ\tilde\Theta\\
		(\alpha_L\circ\gamma_{H_L}\otimes\alpha_R\circ\gamma_{H_R})\circ\tilde\Theta&=\Ad{A_1}\circ \alpha_L\otimes\alpha_R\circ\Theta\circ\gamma_H.
	\end{align}
	If we now define the automorphism $\tilde{\beta}_{g,2}=\gamma^{-1}_H\beta_{g,1}\gamma_H$ then this indeed satisfies that $\omega\circ\gamma_H\circ\tilde{\beta}_{g,2}=\omega\circ\gamma_H$. Furthermore we also have that there exists a $\tilde V_{2,g}\in\UU(\AA),A_{2,g}^{\sigma}\in\UU(\AA_\sigma)$ and some $\tilde{\eta}_g^{L/R}\in\Aut{\AA_{C_{\theta_2}}\cap\AA_{L/R}}$ such that
	\begin{align}
	\gamma_{H}^{-1}\tilde{\beta}_g\gamma_H&=\Ad{\tilde V_{2,g}}\circ\tilde{\eta}_g^L\otimes\tilde{\eta}_g^R\circ\beta_g^U\\
	\tilde{\eta}_g^\sigma&=\Ad{A_{2,g}^\sigma}\circ\gamma_{H_{\sigma}}^{-1}\circ\eta_g^\sigma\circ\beta_g^{\sigma U}\circ\gamma_{H_\sigma}\circ(\beta_g^{\sigma U})^{-1}.
	\end{align}
	...
	\begin{enumerate}
		...
		we have that
		\begin{equation}
			\Ad{\phi(x_g)}\circ\pi_0=\pi_0\circ\alpha_0\circ\Theta\circ\gamma_H\circ(\gamma_{H_L}\otimes\gamma_{H_R})^{-1}\circ\xi_g\circ\beta_g^U\circ(\gamma_{H_L}\otimes\gamma_{H_R})\circ\gamma_H^{-1}\circ\alpha_0^{-1}\circ\Theta^{-1}.
		\end{equation}
		\item for any $A\in\UU(\AA_\sigma)$ we have
		\begin{equation}
			\phi(\pi_0\circ\alpha_0\circ\Theta(A))=\pi_0\circ\alpha_0\circ\Theta\circ\gamma_H\circ\gamma_{H_\sigma}^{-1}(A).
		\end{equation}
		\item $\pi(v)=v.$
		\item $\phi(a)\phi(b)=\phi(ab).$
	\end{enumerate}
	The following operators now belong to $\omega\circ\gamma_H:$
	\begin{align}
	...
	\end{align}
	where $\delta^\sigma_g=\pi_0\circ\alpha_0\circ\Theta\circ\gamma_{H_\sigma}(A^\sigma_{2,g}).$ The fact that the index is invariant under $\Ad{\pi_0(A_1)}$ and under $\phi(\cdot)$ should be rather clear. The proof that it is invariant under the transformation with de $\delta^\sigma_g$ is less straightforward. This proof should be analogous to the proof of lemma \ref{lem:InvarianceUnderChoiceBeta} with one clear difference. The tensor product in the proof is not taken over $\BB(\HH_L)\otimes\BB(\HH_R)$ like in the previous proof but in stead it is over $\pi_0\circ\alpha_0\circ\Theta(\AA_L)''\otimes \pi_0\circ\alpha_0\circ\Theta(\AA_R)''\subset\BB(\HH)$. The actual calculations however should be the same.
\end{proof}
\bibliography{TSPT}
