...
\begin{definition}
For any monotomically decreasing positive function $f:\NN\rightarrow\RR^+$ we define an $f$-norm on the space of interactions by
\begin{equation}
...
\end{equation}
The goal of this section is now to show that the index we constructed is invariant under automorphisms generated by interactions of this form (see theorem \ref{thrm:IndexInvariantUnderLGA}).
...
\section{Properties of locally generated automorphisms:}
 ...
 \end{align}
 where $0<\theta<\pi/2$ and $0<\theta_1<\theta_2<\pi_2$. Now we will define some classes of automorphisms:
 \begin{definition}
 	Take $\alpha\in\Aut{\AA}$ such that $\alpha\circ\beta_g=\beta_g\circ\alpha$ for any $g\in G.$ We say that $\alpha\in\textrm{GSQAut}(\AA)$ if and only if for any $0<\theta_{0.8}<\theta_{1}<\theta_{1.2}<\theta_{1.8}<\theta_{2}<\theta_{2.2}<\theta_{2.8}<\theta_3<\theta_{3.2}<\pi/2$ there exist $a\in\AA$, $\alpha_{[0,\theta_1], \sigma}\in\Aut{\AA_{C_{[0,\theta_1]}}\cap\AA_\sigma}$, $\alpha_{]\theta_1,\theta_2],\sigma,\rho}\in\Aut{\AA_{C_{]\theta_1,\theta_2]}}\cap\AA_{\rho}\cap\AA_{\sigma}}$, $\alpha_{]\theta_2,\theta_3],\sigma,\rho}\in\Aut{\AA_{C_{]\theta_2,\theta_3]}}\cap\AA_{\rho}\cap\AA_{\sigma}}$, $\alpha_{]\theta_3,\pi/2],\rho}\in\Aut{\AA_{C_{[\theta_3,\pi/2]}}\cap\AA_\rho}$, $\alpha_{]\theta_{0.8},\theta_{1.2}],\sigma,\rho}\in\Aut{\AA_{C_{]\theta_{0.8},\theta_{1.2}]}}\cap\AA_{\rho}\cap\AA_{\sigma}}$, $\alpha_{]\theta_{1.8},\theta_{2.2}],\sigma,\rho}\in\Aut{\AA_{C_{]\theta_{1.8},\theta_{2.2}]}}\cap\AA_{\rho}\cap\AA_{\sigma}}$ and  $\alpha_{]\theta_{2.8},\theta_{3.2}],\sigma,\rho}\in\Aut{\AA_{C_{]\theta_{2.8},\theta_{3.2}]}}\cap\AA_{\rho}\cap\AA_{\sigma}}$ for any $\rho\in\{U,D\}$ and $\sigma\in\{L,R\}$ that all commute with $\beta_g^U$ such that
 	\begin{equation}
 		\alpha=\Ad{a}\circ \bigotimes_{\sigma}\alpha_{[0,\theta_1],\sigma}\circ\bigotimes_{\sigma,\rho}\alpha_{]\theta_1,\theta_2],\sigma,\rho}\circ\bigotimes_{\rho}\alpha_{[\theta_3,]}
 	\end{equation}
 \end{definition}
...
\begin{lemma}\label{lem:Bound1}
	Take $0<\theta<\pi/2$. Take $H$ to be an arbitrary $t-$dependent $f-$local $G-$invariant interaction. Take $\gamma_t$ to be the automorphism generated by $H$ up to time $t$. Furthermore, for any $0<n$ take $\Phi_{n}$ to be an $f-$local $G-$invariant interaction such that if $I\cap (I_\theta\cap B_{n}^c)=\emptyset$ then $\Phi_{n}(I)=0$. The following two statements are now true:
	\begin{enumerate}
		\item There exists an $\varepsilon_{1}(n,t)>0$ such that
		\begin{equation}
			\norm{\sum_{I\subset \ZZ^2}\left(\gamma_t(\Phi_{n}(I))-\beta_g^U(\gamma_t(\Phi_{n}(I)))\right)}\leq \varepsilon_{1}(n,t).
		\end{equation}
		\item Take $\theta_2\in]\theta_1,\pi/2[$ then there exists an $\varepsilon_{2}(n,t)>0$ such that for any $u\in\UU(\AA_{C_{\theta_2}^c})$ we have that
		\begin{equation}
			\norm{\sum_{I\subset \ZZ^2}[u,\gamma_t(\Phi_n(I))]}\leq \varepsilon_2(n_1,t).
		\end{equation}
	\end{enumerate}
	For any $t>0$ we have that
	\begin{align}
		\lim_{n\rightarrow\infty}\varepsilon_{1}(n,t)=\lim_{n\rightarrow\infty}\varepsilon_{2}(n,t)=0.
	\end{align}
\end{lemma}
\begin{proof}
	content...
...
\begin{lemma}\label{lem:Bound2}
	Take $0<\theta<\pi/2$. Take $H$ to be an arbitrary $t-$dependent $f$-local interaction. Take $\gamma_t$ to be the automorphism generated by $H$ up to time $t$. Take $\Phi_{n}$ (for every $0<n$) and $\Psi$ $f-$local interactions such that:
	\begin{enumerate}
		\item if $I\cap (I_{\pi/2}\cap B_{n}^c)=\emptyset$ then $\Phi_{n}(I)=0.$
		\item if $I\cap C_{\theta}=\emptyset$ then $\Psi(I)=0.$
	\end{enumerate}
	There now exists an $\varepsilon(n_1,t)$ such that
	\begin{equation}
		\norm{\sum_{I,J\subset\ZZ^2}[\Psi(I),\gamma_t(\Phi_{n}(J))]}\leq\varepsilon(n,t).
	\end{equation}
	For any $t>0$ we have that
	\begin{equation}
		\lim_{n\rightarrow\infty}\varepsilon(n,t)=0.
	\end{equation}
\end{lemma}
\begin{proof}
	content...
...
\begin{lemma}\label{lem:Bound3}
	Take $0<\theta<\pi/2$. Take $H$ to be an arbitrary $t-$dependent $f$-local interaction. Take $\gamma_t$ to be the automorphism generated by $H$ up to time $t$. For any $0<n$ take $\Phi_{n/2}$ and $\Psi_{n}$ to be $f-$local interactions such that
	\begin{enumerate}
		\item if $I\cap (I_\theta\cap B_{n/2})=\emptyset$ then $\Phi_{n/2}(I)=0.$
		\item if $I\cap B_{n}^c=\emptyset$ then $\Psi_{n}(I)=0.$
	\end{enumerate}
	There now exists an $\varepsilon(n,t)$ such that
	\begin{equation}
		\norm{\sum_{I,J\subset\ZZ^2}[\Psi_{n}(I),\gamma_t(\Phi_{n/2}(J))]}\leq \varepsilon(n,t).
	\end{equation}
	For any $t>0$ we have that
	\begin{equation}
		\lim_{n\rightarrow\infty}\varepsilon(n_0,t)=0.
	\end{equation}
\end{lemma}
\begin{proof}
	content...
