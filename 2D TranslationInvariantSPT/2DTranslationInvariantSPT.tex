...
\begin{proof}
	...
	and take similarly
	\begin{equation}
		w_n\defeq \mathcal{T}\exp(-i\int_0^1 \d s \sum_{I\subset B_n}H_{(C_{\theta_1}\cup \tau(C_{\theta_1}))^c}(s,I)).
	\end{equation}
	We claim that there exists an $a_1\in\UU(\AA)$ such that
	\begin{equation}
		\lim_{n\rightarrow\infty}\norm{u_n^\dagger\Theta(u_n w_n^\dagger)w_n-a_1}=0.
	\end{equation}
	Clearly by construction this $a_1$ satisfies equation \eqref{eq:TwoAngleLemmaEquation1}. We will show that this sequence is a Cauchy sequence. Take $n_0$ then $n,m>n_0$ we have that
	\begin{equation}
		\text{{\color{red}This I still have to do.}}
	\end{equation}
\end{proof}
