...
\begin{proof}
	...
	\end{equation}
	and observe that we can take $A\defeq \nu^{-1}(A_\sigma^\dagger)A_\sigma$. Now we will prove that $A\in\AA_{W(C_\theta)^c\cap\sigma}=\AA_{(W(C_\theta)^c\cap\sigma)^c}'$. Take $X\in\AA_{(W(C_\theta)^c\cap\sigma)^c}$ arbitrary then we have that
	\begin{align}
		\Ad{A}(X)&=\nu^{-1}\circ\Ad{A_\sigma^\dagger}\circ\nu\circ\Ad{A_\sigma}\\
		&=
	\end{align}
\end{proof}
\begin{theorem}
