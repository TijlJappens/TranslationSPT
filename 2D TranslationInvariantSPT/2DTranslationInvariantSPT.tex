...
\begin{theorem}\label{thrm:ExistenceFirstIndex}
	...
	\begin{enumerate}
		...
		\begin{equation}
			\textrm{Index}^{\AA,U(g)}(\omega)=\textrm{Index}^{\AA,U(g)}(\omega\circ\gamma^{\Phi}_{0;1}).
		\end{equation}
		\item For $\phi$, any a one dimensional $G$-invariant state satisfying assumption \ref{assumption1d}, the (infinite) tensor product of this state in the vertical direction (see section \ref{sec:OneTranslationDirectionExample} definition \ref{def:InfiniteTensorProductState}) satisfies
		\begin{equation}
			\textrm{Index}^{\AA_{\text{1d}},U(g)}_{1d}(\phi)=\textrm{Index}^{\AA,U(g)}(\omega).
		\end{equation}
	\end{enumerate}
