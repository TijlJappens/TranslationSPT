...
\end{equation}
be the horizontal cone. We will use $\tau$ to denote the automorphism that moves every element of $\AA$ one site upward. Of course this is only an automorphism if the $C^*$ algebra is translation invariant in the vertical direction which we will assume. We will sometimes also denote $\tau(Z)$ for some $Z\subset\ZZ^2$ to also denote the set that has been moved up by one site without using a different symbol for it. In what follows we will sometimes need to widen our cone vertically by one site for this purpose we define $W$ such that
\begin{equation}
...
\begin{assumption}
	...
	\end{enumerate}
\end{assumption}
Take $(\HH_0=\HH_L\otimes\HH_R,\pi_0=\pi_L\otimes\pi_R,\Omega_0)$ a GNS triple of $\omega_0$. Because we will be working with states that are translation invariant it make sense to define the translation action on the GNS space of $\omega_0$:
