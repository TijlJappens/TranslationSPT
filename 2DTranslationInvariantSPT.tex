...
We say that a linear functional $\omega:\AA\rightarrow \CC$ is a state if it is positive and normalized. This means that for any $A\in\AA$, $\omega(A^\dagger A)\geq 0$ and that $\omega(\id)=1$. We denote the set of states on $\AA$ by $\mathcal{S}(\AA)$. In this paper, we will work exclusively with states that are pure. In this setting, $\omega\in\mathcal{S}(\AA)$ being pure means that it is extremal in the sense that for any $p\in]0,1[$, the only two other states $\omega_1,\omega_2\in\mathcal{S}(\AA)$ that satisfy
\begin{equation}
...
\begin{theorem}\label{thrm:ExistenceFirstIndex}
	...
	\begin{enumerate}
		...
		\end{equation}
		\item For $\phi\in\PP(\AA_{L_0})$, any one dimensional $G$-invariant state satisfying assumption \ref{assumption1d}, the state $\omega_\phi$ as defined in definition \ref{def:InfiniteTensorProduct} satisfies
		\begin{equation}
...
\begin{theorem}\label{thrm:ExistenceSecondIndex}
	...
	\begin{enumerate}
		...
		\end{equation}
		\item For $\phi\in\PP(\AA_{L_0})$, any one dimensional $G$-invariant and translation invariant state satisfying assumption \ref{assumption1d}, the state $\omega_\phi$ as defined in \ref{def:InfiniteTensorProduct} satisfies
		\begin{equation}
...
\end{restatable}
\begin{proof}
	See appendix \ref{sec:ProofOfMainResult1}.
\end{proof}
...
\clearpage
\appendix
\section{Proof of lemmas \ref{lem:SRE_Implies_QDisentanglable_OneTranslation}, \ref{lem:SRE_Implies_Assumption_1d} and \ref{lem:SRE_Implies_Assumption_1d}}\label{sec:ProofOfMainResult1}
...
\end{proof}
Lastly, we prove lemma \ref{lem:SRE_Implies_QDisentanglable_TwoTranslations}, which we restate here for convenience:
