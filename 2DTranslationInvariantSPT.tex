...
\end{proof}
Due to all these considerations we will write the $H^2(G,\TT)$ and $H^1(G,\TT)$-valued indices as $\textrm{Index}^{\AA,U}(\omega)$ and $\textrm{Index}^{\AA,U}_{\text{2 trans}}(\omega)$ from here on onward.
\section{Examples}
...
\end{proof}
\subsection{Example with non-trivial $H^2(G,\TT)$-valued index}

By the result of \cite{ogata2019classification}, lemma \ref{lem:phiExampleConsistentWithConjectureSplitProperty} implies that $\phi$ has a well defined 1d SPT index (see appendix \ref{sec:OneDimensionalIndices}). For any $j\in\ZZ$ and $n\in\NN$ take
\begin{align}
...
\subsection{Example with non-trivial $H^1(G,\TT)$-valued index}
