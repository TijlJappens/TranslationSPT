...
\begin{restatable}{assumption}{assumptionTwo}\label{assumption:2Translations}
	Let $\omega\in\PP(\AA)$ satisfy assumption \ref{assumption} with the additional properties that
	\begin{enumerate}
		\item $\eta^L_g\in\Aut{\AA_{\nu^{-1}(C_\theta\cap L)}}$ and  $\eta^R_g\in\Aut{\AA_{\nu(C_\theta\cap R)}}$.
		\item $\omega$ is also translation invariant in the other direction ($\omega\circ\nu=\omega$).
	\end{enumerate}
\end{restatable}
...
\begin{definition}\label{def:ConeOperators}
	...
	\end{align}
	If additionally $\xi\in\Aut{\AA_{C_\theta\cap\sigma}}$ we will say that $x\in\textrm{Cone}_\sigma^W(\alpha_0,\theta)$.
\end{definition}
\begin{proof}
...
\end{lemma}
\begin{proof}
	The proof is presented after lemma \ref{lem:Definition2CochainAppendix}.
\end{proof}
\begin{lemma}
...
\end{lemma}
\begin{proof}
	The proof is presented after \ref{lem:DefinitionAlphaAppendix}.
\end{proof}
...
\end{lemma}
\begin{proof}
	The proof is given after lemma \ref{lem:TransformationUnderDeltaTwoTranslationsAppendix}.
\end{proof}
We will now show that the index is independent on the choice of $\tilde{\beta}_g$ and its decomposition.
